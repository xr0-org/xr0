...
callstack_create(struct callstack *prev, enum path_state caller_state,
		struct ast_function *func);

static struct callstack *
callstack_copy(struct callstack *old);

static struct callstack *
callstack_prev(struct callstack *);

static enum path_state
callstack_state(struct callstack *);

static enum path_state
callstack_basestate(struct callstack *);

static struct ast_function *
callstack_func(struct callstack *);

static int
callstack_len(struct callstack *);

struct path {
	enum path_state path_state;
	struct state *abstract, *actual, *param;
...
static int
path_basedepth(struct path *p)
{
	return p->stack ? callstack_len(p->stack) : 0;
}

enum path_state
path_callerorigstate(struct path *p)
{
	return p->stack ? callstack_basestate(p->stack) : p->path_state;
}

static void
path_pushcall(struct path *p, struct ast_function *f)
{
	p->stack = callstack_create(p->stack, p->path_state, f);
}

static void
path_popcall(struct path *p)
{
	assert(p->stack);
	/* TODO: clean up head */
	p->path_state = callstack_state(p->stack);
	p->stack = callstack_prev(p->stack);
}
...
char *
path_setupcall_str(struct path *);

char *
path_call_str(struct path *);
...
path_str(struct path *p)
{
	...
	case PATH_STATE_SETUPCALL:
		return path_setupcall_str(p);
	case PATH_STATE_CALL:
		return path_call_str(p);
...
char *
path_setupcall_abstract_str(struct path *);

char *
path_setupcall_actual_str(struct path *);

char *
path_setupcall_str(struct path *p)
{
	switch (path_callerorigstate(p)) {
	case PATH_STATE_ABSTRACT:
		return path_setupcall_abstract_str(p);
	case PATH_STATE_ACTUAL:
		return path_setupcall_actual_str(p);
	default:
		assert(false);
	}
}
...
char *
path_setupcall_abstract_str(struct path *p)
{
	struct strbuilder *b = strbuilder_create();
	strbuilder_printf(b, "phase:\tSETUP VERIFY (ABSTRACT)\n\n");
	strbuilder_printf(b, "text:\n%s\n", state_programtext(p->param));
	strbuilder_printf(b, "%s\n", state_str(p->param));
	return strbuilder_build(b);
}
...
char *
path_setupcall_actual_str(struct path *p)
{
	struct strbuilder *b = strbuilder_create();
	strbuilder_printf(b, "phase:\tSETUP VERIFY (ACTUAL)\n\n");
	strbuilder_printf(b, "text:\n%s\n", state_programtext(p->param));
	strbuilder_printf(b, "%s\n", state_str(p->param));
	return strbuilder_build(b);
}
...
char *
path_call_abstract_str(struct path *);

char *
path_call_actual_str(struct path *);

char *
path_call_str(struct path *p)
{
	switch (callstack_state(p->stack)) {
	case PATH_STATE_ABSTRACT:
		return path_call_abstract_str(p);
	case PATH_STATE_ACTUAL:
		return path_call_actual_str(p);
	default:
		assert(false);
	}
}
...
char *
path_call_abstract_str(struct path *p)
{
	struct strbuilder *b = strbuilder_create();
	strbuilder_printf(b, "phase:\tCALL (ABSTRACT)\n\n");
	strbuilder_printf(b, "text:\n%s\n", state_programtext(p->abstract));
	strbuilder_printf(b, "%s\n", state_str(p->abstract));
	return strbuilder_build(b);
}
...
char *
path_call_actual_str(struct path *p)
{
	struct strbuilder *b = strbuilder_create();
	strbuilder_printf(b, "phase:\tCALL (ACTUAL)\n\n");
	strbuilder_printf(b, "text:\n%s\n", state_programtext(p->actual));
	strbuilder_printf(b, "%s\n", state_str(p->actual));
	return strbuilder_build(b);
}
...
static struct error *
path_progress_setupcall(struct path *p, progressor *);

static struct error *
path_progress_call(struct path *p, progressor *);
...
progressact(struct path *p, progressor *prog)
{
	...
	case PATH_STATE_SETUPCALL:
		return path_progress_setupcall(p, prog);
	case PATH_STATE_CALL:
		return path_progress_call(p, prog);
	default:
		assert(false);
	}
}
...
{
	if (state_atsetupend(p->abstract, path_basedepth(p))) {
...
{
	if (state_atend(p->abstract, path_basedepth(p))) {
...
{
	if (state_atsetupend(p->actual, path_basedepth(p))) {
...
{	
	if (state_atend(p->actual, path_basedepth(p))) {
...
static struct error *
path_progress_setupcall_abstract(struct path *, progressor *);

static struct error *
path_progress_setupcall_actual(struct path *, progressor *);

static struct error *
path_progress_setupcall(struct path *p, progressor *prog)
{
	switch (path_callerorigstate(p)) {
	case PATH_STATE_ABSTRACT:
		return path_progress_setupcall_abstract(p, prog);
	case PATH_STATE_ACTUAL:
		return path_progress_setupcall_actual(p, prog);
	default:
		assert(false);
	}
}
...
static struct error *
path_progress_setupcall_abstract(struct path *p, progressor *prog)
{
	if (state_atsetupend(p->param, 0)) {
		assert(!state_atend(p->param, 0));
		state_popframe(p->param);
		p->path_state = PATH_STATE_CALL;
		return ast_function_setupverify(
			callstack_func(p->stack), p->param, p->abstract
		);
	}
	return progressortrace(p->param, prog);
}


static struct error *
path_progress_setupcall_actual(struct path *p, progressor *prog)
{
	if (state_atsetupend(p->param, 0)) {
		assert(!state_atend(p->param, 0));
		state_popframe(p->param);
		p->path_state = PATH_STATE_CALL;
		return ast_function_setupverify(
			callstack_func(p->stack), p->param, p->actual
		);
	}
	return progressortrace(p->param, prog);
}
...
static struct error *
path_progress_call_abstract(struct path *, progressor *);

static struct error *
path_progress_call_actual(struct path *, progressor *);

static struct error *
path_progress_call(struct path *p, progressor *prog)
{
	switch (path_callerorigstate(p)) {
	case PATH_STATE_ABSTRACT:
		return path_progress_call_abstract(p, prog);
	case PATH_STATE_ACTUAL:
		return path_progress_call_actual(p, prog);
	default:
		assert(false);
	}
}
...
static struct error *
path_progress_call_abstract(struct path *p, progressor *prog)
{
	if (state_atend(p->abstract, path_basedepth(p))) {
		path_popcall(p);
		return NULL;
...
static struct error *
path_progress_call_actual(struct path *p, progressor *prog)
{
	if (state_atend(p->actual, path_basedepth(p))) {
		path_popcall(p);
		return NULL;
...
static void
abstract_copywithsplit(struct path *old, struct path *new, struct map *);

static void
actual_copywithsplit(struct path *old, struct path *new, struct map *);
...
path_copywithsplit(struct path *old, struct map *split)
{
	...
	case PATH_STATE_SETUPABSTRACT:
	case PATH_STATE_ABSTRACT:
		abstract_copywithsplit(old, p, split);
		break;
	case PATH_STATE_SETUPACTUAL:
	case PATH_STATE_ACTUAL:
		actual_copywithsplit(old, p, split);
		break;
	case PATH_STATE_SETUPCALL:
		assert(old->param);
		p->param = state_copywithname(old->param, fname);
		if (!state_split(p->param, split)) {
			p->path_state = PATH_STATE_ATEND;
		}
		p->stack = callstack_copy(old->stack);
		switch (callstack_state(old->stack)) {
		case PATH_STATE_ABSTRACT:
			p->abstract = state_copywithname(old->abstract, fname);
			state_setrconsts(p->abstract, p->param);
			break;
		case PATH_STATE_ACTUAL:
			p->abstract = state_copywithname(old->abstract, fname);
			state_setrconsts(p->abstract, p->param);
			p->actual = state_copywithname(old->actual, fname);
			state_setrconsts(p->actual, p->param);
			break;
		default:
			assert(false);
		}
...
static void
abstract_copywithsplit(struct path *old, struct path *new, struct map *split)
{
	char *fname = ast_function_name(new->f);
	new->abstract = state_copywithname(old->abstract, fname);
	if (!state_split(new->abstract, split)) {
		new->path_state = PATH_STATE_ATEND;
	}
}

static void
actual_copywithsplit(struct path *old, struct path *new, struct map *split)
{
	char *fname = ast_function_name(new->f);
	new->actual = state_copywithname(old->actual, fname);
	if (!state_split(new->actual, split)) {
		new->path_state = PATH_STATE_ATEND;
	}
	new->abstract = state_copywithname(old->abstract, fname);
	state_setrconsts(new->abstract, new->actual);
}


static struct error *
path_callabstract(struct path *, struct ast_expr *);

static struct error *
path_callactual(struct path *, struct ast_expr *);

static struct error *
path_call(struct path *p, struct ast_expr *call)
{
	switch (path_callerorigstate(p)) {
	case PATH_STATE_ABSTRACT:
		return path_callabstract(p, call);
	case PATH_STATE_ACTUAL:
		return path_callactual(p, call);
	default:
		assert(false);
	}
}
...
static struct error *
path_callabstract(struct path *p, struct ast_expr *call)
{
	struct error *err;

	if ((err = ast_expr_callprepare(call, p->abstract))) {
		assert(!error_to_pathinstruct(err));
		return err;
	}

	struct ast_function *f = externals_getfunc(
		state_getext(p->abstract),
	...
	p->path_state = PATH_STATE_SETUPCALL;
	...
	p->param = state_create(frame, state_getext(p->abstract));
	if ((err = ast_function_initparams(f, p->param))) {
		assert(!error_to_pathinstruct(err));
		return err;
	}
	state_setrconsts(p->param, p->abstract);
	struct frame *setupframe = frame_setup_create(
		"setup",
		ast_function_abstract(f),
		EXEC_ABSTRACT_SETUP_ONLY
	);
	state_pushframe(p->param, setupframe);

	return NULL;
}

static struct error *
path_callactual(struct path *p, struct ast_expr *call)
{
	struct error *err;

	if ((err = ast_expr_callprepare(call, p->actual))) {
		assert(!error_to_pathinstruct(err));
		return err;
	}

	struct ast_function *f = externals_getfunc(
		state_getext(p->actual),
		ast_expr_as_identifier(ast_expr_call_root(call))
	);
	assert(f);
	...
	p->param = state_create(frame, state_getext(p->actual));
	if ((err = ast_function_initparams(f, p->param))) {
		assert(!error_to_pathinstruct(err));
		return err;
	}
	state_setrconsts(p->param, p->actual);
	struct frame *setupframe = frame_setup_create(
		"setup",
		ast_function_abstract(f),
		EXEC_ABSTRACT_SETUP_ONLY
...
static struct lexememarker *
path_call_lexememarker(struct path *);
...
path_lexememarker(struct path *p)
{
	...
	case PATH_STATE_SETUPCALL:
		return state_lexememarker(p->param);
	case PATH_STATE_CALL:
		return path_call_lexememarker(p);
	case PATH_STATE_UNINIT:
	case PATH_STATE_HALFWAY:
	case PATH_STATE_AUDIT:
	case PATH_STATE_ATEND:
...
static struct lexememarker *
path_call_lexememarker(struct path *p)
{
	switch (path_callerorigstate(p)) {
	case PATH_STATE_ABSTRACT:
		return state_lexememarker(p->abstract);
...
callstack_create(struct callstack *prev, enum path_state caller_state,
		struct ast_function *f)
{
	struct callstack *s = malloc(sizeof(struct callstack));
	assert(s);
	s->prev = prev;
	s->caller_state = caller_state;
	s->f = f;
	return s;
}
...
callstack_copy(struct callstack *old)
{
	return callstack_create(
		old->prev ? callstack_copy(old->prev) : NULL,
		old->caller_state,
		ast_function_copy(old->f)
	);
}

static struct callstack *
callstack_prev(struct callstack *s)
{
	return s->prev;
}

static enum path_state
callstack_state(struct callstack *s)
{
	return s->caller_state;
}

static enum path_state
callstack_basestate(struct callstack *s)
{
	return s->prev ? callstack_basestate(s->prev) : s->caller_state;
}

static struct ast_function *
callstack_func(struct callstack *s)
{
	return s->f;
}

static int
callstack_len(struct callstack *s)
{
	return 1 + (s->prev ? callstack_len(s->prev) : 0);
}
...
int
pathinstruct_shouldadvancepc(struct pathinstruct *inst)
{
	switch (inst->type) {
	case PATHINSTRUCT_SPLIT:
		return 0;
...
char *
pathinstruct_str(struct pathinstruct *inst)
{
	struct strbuilder *b = strbuilder_create();
	strbuilder_printf(b, "pathinstruct{");
	switch (inst->type) {
	case PATHINSTRUCT_SPLIT:
		strbuilder_printf(b, "split");
		break;
	case PATHINSTRUCT_CALL: {
		char *s = ast_expr_str(inst->call);
		strbuilder_printf(b, "call: %s", s);
		free(s);
		break;
	}
	default:

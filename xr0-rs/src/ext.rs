...
pub unsafe extern "C" fn externals_create() -> *mut externals {
    let mut ext: *mut externals = malloc(
        ::core::mem::size_of::<externals>() as libc::c_ulong,
    ) as *mut externals;
    (*ext).func = map_create();
    (*ext).var = map_create();
...
pub unsafe extern "C" fn externals_types_str(
    ...
    while i_0 < (*m).n {
        let mut type_1: *mut libc::c_char = ast_type_str(
            (*((*m).entry).offset(i_0 as isize)).value as *mut ast_type,
        );
        strbuilder_printf(
            b,
            b"%s%s\n\0" as *const u8 as *const libc::c_char,
...
#[no_mangle]
pub unsafe extern "C" fn externals_declarestruct(
    mut ext: *mut externals,
    mut t: *mut ast_type,
) {

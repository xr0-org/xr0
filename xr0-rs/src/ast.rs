...
use crate::util::{
    ...
    string_arr_create, string_arr_deque, string_arr_n, string_arr_s, Map,
};
use crate::value::{
...
pub type AstTypeBase = libc::c_uint;
pub const TYPE_USERDEF: AstTypeBase = 14;
pub const TYPE_ENUM: AstTypeBase = 13;
pub const TYPE_UNION: AstTypeBase = 12;
pub const TYPE_STRUCT: AstTypeBase = 11;
pub const TYPE_ARRAY: AstTypeBase = 10;
pub const TYPE_POINTER: AstTypeBase = 9;
pub const TYPE_UNSIGNED: AstTypeBase = 8;
pub const TYPE_SIGNED: AstTypeBase = 7;
pub const TYPE_DOUBLE: AstTypeBase = 6;
pub const TYPE_FLOAT: AstTypeBase = 5;
pub const TYPE_LONG: AstTypeBase = 4;
pub const TYPE_INT: AstTypeBase = 3;
pub const TYPE_SHORT: AstTypeBase = 2;
pub const TYPE_CHAR: AstTypeBase = 1;
pub const TYPE_VOID: AstTypeBase = 0;
...
pub type AstStmtKind = libc::c_uint;
pub const STMT_ALLOCATION: AstStmtKind = 512;
pub const STMT_JUMP: AstStmtKind = 256;
pub const STMT_ITERATION_E: AstStmtKind = 128;
pub const STMT_ITERATION: AstStmtKind = 64;
pub const STMT_SELECTION: AstStmtKind = 32;
pub const STMT_EXPR: AstStmtKind = 16;
pub const STMT_COMPOUND_V: AstStmtKind = 8;
pub const STMT_COMPOUND: AstStmtKind = 4;
pub const STMT_LABELLED: AstStmtKind = 2;
pub const STMT_NOP: AstStmtKind = 1;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct decision {
    pub decision: bool,
...
pub type AstExternDeclKind = libc::c_uint;
pub const EXTERN_STRUCT: AstExternDeclKind = 3;
pub const EXTERN_TYPEDEF: AstExternDeclKind = 2;
pub const EXTERN_VARIABLE: AstExternDeclKind = 1;
pub const EXTERN_FUNCTION: AstExternDeclKind = 0;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct Ast {
    pub n: libc::c_int,
...
pub unsafe fn ast_expr_decide(mut expr: *mut AstExpr, mut state: *mut State) -> bool {
    match ast_expr_kind(expr) {
        AstExprKind::Constant => ast_expr_as_constant(expr) != 0,
        AstExprKind::Unary => expr_unary_decide(expr, state),
        AstExprKind::IsDeallocand => expr_isdeallocand_decide(expr, state),
        AstExprKind::Binary => expr_binary_decide(expr, state),
        _ => panic!(),
    }
}
...
pub unsafe fn ast_expr_rangedecide(
    ...
    match ast_expr_kind(expr) {
        AstExprKind::Unary => return unary_rangedecide(expr, lw, up, state),
...
unsafe fn reduce_assume(
    ...
    match (*expr).kind {
        AstExprKind::Identifier => return identifier_assume(expr, value, s),
        AstExprKind::Unary => {
            assert_eq!(ast_expr_unary_op(expr), AstUnaryOp::Bang);
            return reduce_assume(ast_expr_unary_operand(expr), !value, s);
        }
        AstExprKind::Bracketed => return reduce_assume((*expr).root, value, s),
        AstExprKind::Call | AstExprKind::StructMember => {
            return ast_expr_pf_reduce_assume(expr, value, s)
        }
        AstExprKind::Binary => return binary_assume(expr, value, s),
        _ => {
            panic!();
        }
    }
...
pub unsafe fn ast_expr_assignment_create(
    ...
    let mut expr: *mut AstExpr = ast_expr_create();
    (*expr).kind = AstExprKind::Assignment;
    (*expr).root = root;
...
unsafe fn expr_to_binary(mut expr: *mut AstExpr) -> *mut AstExpr {
    match ast_expr_kind(expr) {
        AstExprKind::Binary => return ast_expr_copy(expr),
        _ => {
...
pub unsafe fn ast_expr_eval(mut expr: *mut AstExpr, mut state: *mut State) -> *mut result {
    match ast_expr_kind(expr) {
        AstExprKind::Constant => expr_constant_eval(expr, state),
        AstExprKind::StringLiteral => expr_literal_eval(expr, state),
        AstExprKind::Identifier => expr_identifier_eval(expr, state),
        AstExprKind::Unary => expr_unary_eval(expr, state),
        AstExprKind::StructMember => expr_structmember_eval(expr, state),
        AstExprKind::Call => expr_call_eval(expr, state),
        AstExprKind::Assignment => expr_assign_eval(expr, state),
        AstExprKind::IncDec => expr_incdec_eval(expr, state),
        AstExprKind::Binary => expr_binary_eval(expr, state),
...
pub unsafe fn ast_expr_lvalue(mut expr: *mut AstExpr, mut state: *mut State) -> LValueRes {
    match ast_expr_kind(expr) {
        AstExprKind::Identifier => return expr_identifier_lvalue(expr, state),
        AstExprKind::Unary => return expr_unary_lvalue(expr, state),
        AstExprKind::StructMember => return expr_structmember_lvalue(expr, state),
        _ => {
            panic!();
        }
    }
...
pub unsafe fn ast_expr_absexec(mut expr: *mut AstExpr, mut state: *mut State) -> *mut result {
    match ast_expr_kind(expr) {
        AstExprKind::Assignment => assign_absexec(expr, state),
        AstExprKind::IsDereferencable => isdereferencable_absexec(expr, state),
        AstExprKind::Allocation => alloc_absexec(expr, state),
        AstExprKind::Identifier
        | AstExprKind::Constant
        | AstExprKind::Unary
        | AstExprKind::Call
...
pub unsafe fn ast_expr_literal_create(mut s: *mut libc::c_char) -> *mut AstExpr {
    let mut expr: *mut AstExpr = ast_expr_create();
    (*expr).kind = AstExprKind::StringLiteral;
    (*expr).u.string = s;
    return expr;
}
...
pub unsafe fn ast_expr_constant_create_char(mut c: libc::c_char) -> *mut AstExpr {
    let mut expr: *mut AstExpr = ast_expr_create();
    (*expr).kind = AstExprKind::Constant;
...
pub unsafe fn ast_expr_identifier_create(mut s: *mut libc::c_char) -> *mut AstExpr {
    let mut expr: *mut AstExpr = ast_expr_create();
    (*expr).kind = AstExprKind::Identifier;
    (*expr).u.string = s;
    return expr;
}
...
pub unsafe fn ast_expr_pf_reduce(mut e: *mut AstExpr, mut s: *mut State) -> *mut result {
    match ast_expr_kind(e) {
        AstExprKind::Constant | AstExprKind::StringLiteral | AstExprKind::Identifier => {
            ast_expr_eval(e, s)
        }
        AstExprKind::Unary => unary_pf_reduce(e, s),
        AstExprKind::Binary => binary_pf_reduce(
            ast_expr_binary_e1(e),
            ast_expr_binary_op(e),
            ast_expr_binary_e2(e),
            s,
        ),
        AstExprKind::Call => call_pf_reduce(e, s),
        AstExprKind::StructMember => structmember_pf_reduce(e, s),
...
pub unsafe fn ast_expr_member_create(
    ...
    let mut expr: *mut AstExpr = ast_expr_create();
    (*expr).kind = AstExprKind::StructMember;
    (*expr).root = _struct;
    (*expr).u.string = field;
    return expr;
}
...
pub unsafe fn ast_expr_copy(mut expr: *mut AstExpr) -> *mut AstExpr {
    ...
    match (*expr).kind {
        ...
        AstExprKind::Bracketed => return ast_expr_bracketed_create(ast_expr_copy((*expr).root)),
        AstExprKind::Call => return ast_expr_copy_call(expr),
        AstExprKind::IncDec => {
        ...
        AstExprKind::IsDeallocand => {
            return ast_expr_isdeallocand_create(ast_expr_copy((*expr).root))
        }
        AstExprKind::IsDereferencable => {
            return ast_expr_isdereferencable_create(ast_expr_copy((*expr).root))
        }
        AstExprKind::ArbArg => return ast_expr_arbarg_create(),
        AstExprKind::Allocation => return ast_expr_alloc_copy(expr),
        _ => {
            panic!();
        }
    }
...
unsafe fn ast_expr_binary_str_build(mut expr: *mut AstExpr, mut b: *mut StrBuilder) {
    let opstr = match (*expr).u.binary.op {
...
unsafe fn ast_expr_unary_str_build(mut expr: *mut AstExpr, mut b: *mut StrBuilder) {
    let mut op: AstUnaryOp = (*expr).u.unary_op;
    let c = match op {
...
pub unsafe fn ast_expr_destroy(mut expr: *mut AstExpr) {
    match (*expr).kind {
        ...
        AstExprKind::Constant | AstExprKind::ArbArg => {}
        AstExprKind::Allocation => {
...
pub unsafe fn ast_expr_getfuncs(mut expr: *mut AstExpr) -> Box<string_arr> {
    match (*expr).kind {
        ...
        AstExprKind::Call => ast_expr_call_getfuncs(expr),
        AstExprKind::Bracketed | AstExprKind::Unary | AstExprKind::IncDec => {
            ast_expr_getfuncs((*expr).root)
        }
        AstExprKind::Assignment => string_arr_concat(
            &ast_expr_getfuncs((*expr).root),
            &ast_expr_getfuncs((*expr).u.assignment_value),
        ),
        AstExprKind::Binary => string_arr_concat(
...
pub unsafe fn ast_expr_splits(mut e: *mut AstExpr, mut s: *mut State) -> AstStmtSplits {
    match ast_expr_kind(e) {
        AstExprKind::Call => call_splits(e, s),
        AstExprKind::Assignment => ast_expr_splits(ast_expr_assignment_rval(e), s),
        AstExprKind::Unary => ast_expr_splits(ast_expr_unary_operand(e), s),
        AstExprKind::Binary => binary_splits(e, s),
        AstExprKind::IncDec => ast_expr_splits(ast_expr_incdec_root(e), s),
        AstExprKind::StructMember => ast_expr_splits(ast_expr_member_root(e), s),
        AstExprKind::Constant
        | AstExprKind::Identifier
        | AstExprKind::StringLiteral
        | AstExprKind::ArbArg
        | AstExprKind::IsDereferencable
        | AstExprKind::Allocation => AstStmtSplits {
            n: 0 as libc::c_int,
...
pub unsafe fn ast_block_preconds(mut b: *mut AstBlock) -> preconds_result {
...
pub unsafe fn ast_stmt_getfuncs(mut stmt: *mut AstStmt) -> Box<string_arr> {
    match (*stmt).kind as libc::c_uint {
        1 => string_arr_create(),
...
unsafe fn ast_stmt_expr_getfuncs(mut stmt: *mut AstStmt) -> Box<string_arr> {
    ast_expr_getfuncs((*stmt).u.expr)
}
unsafe fn ast_stmt_selection_getfuncs(mut stmt: *mut AstStmt) -> Box<string_arr> {
...
unsafe fn ast_stmt_iteration_getfuncs(mut stmt: *mut AstStmt) -> Box<string_arr> {
    let mut init: *mut AstStmt = (*stmt).u.iteration.init;
    let mut cond: *mut AstStmt = (*stmt).u.iteration.cond;
...
unsafe fn ast_stmt_compound_getfuncs(mut stmt: *mut AstStmt) -> Box<string_arr> {
    let mut res = string_arr_create();
...
pub unsafe fn ast_type_create_ptr(mut ref_0: *mut AstType) -> *mut AstType {
    ...
    let mut t: *mut AstType = ast_type_create(TYPE_POINTER, 0 as AstTypeModifier);
...
pub unsafe fn ast_type_create_arr(mut base: *mut AstType, mut length: libc::c_int) -> *mut AstType {
    ...
    let mut t: *mut AstType = ast_type_create(TYPE_ARRAY, 0 as AstTypeModifier);
...
pub unsafe fn ast_type_create_struct(
    ...
    let mut t: *mut AstType = ast_type_create(TYPE_STRUCT, 0 as AstTypeModifier);
...
pub unsafe fn ast_type_create_userdef(mut name: *mut libc::c_char) -> *mut AstType {
    let mut t: *mut AstType = ast_type_create(TYPE_USERDEF, 0 as AstTypeModifier);
...
pub unsafe fn ast_type_create_struct_partial(mut tag: *mut libc::c_char) -> *mut AstType {
    return ast_type_create_struct(tag, 0 as *mut AstVariableArr);
}
...
pub unsafe fn ast_type_copy_struct(mut old: *mut AstType) -> *mut AstType {
    ...
    let mut new: *mut AstType =
        ast_type_create(TYPE_STRUCT, (*old).mod_0 as libc::c_uint as AstTypeModifier);
...
pub unsafe fn ast_function_preconditions(mut f: *mut AstFunction) -> preconds_result {
